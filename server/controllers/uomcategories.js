const {UomCategory} = require('../models/index');
...
module.exports = {
	...
	add: (req, res) => {
	    ...
	    UomCategory.create(value).then(item => {
          result.status = status;
          result.result = item;
          return res.status(status).send(result);
        })
	...
	show: (req, res) => {
		...
		UomCategory.findByPk(req.params.id)
		.then(item => {
			result.status = status;
			result.result = item;
			return res.status(status).send(result);
	...
	import: (req, res) => {
		...
		UomCategory.sync({
			...
			UomCategory.bulkCreate(req.body)
			.then(affectedRows =>{
				result.status = status;
